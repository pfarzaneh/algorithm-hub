...
class NaiveProductTest {
    ...
    private long doCompute(long[] arr) {
        ...
        long result = naiveProduct.compute(arr);
    ...
    void computeSequentiallyTest_with_large_dynamic_array() {
        ...
        long start = new Date().getTime();
        long result = naiveProduct.computeSequentially(arr);
    ...
    @Test
    void dummyAddTest() {
        int result = naiveProduct.dummyAdd(8, 2);
    ...
    void dummyAddTest(int first, int second, int expectedResult) {
        int result = naiveProduct.dummyAdd(first, second);
